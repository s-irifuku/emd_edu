#load_resource.py
#coding:UTF-8
from main import session, Branch, Department, Device, OS, CPU, Memory, StorageType, StorageCapacity

#[(支店ID, 支店名)]を返す
def load_branch():
    branch_list = []
    for (id, name) in session.query(Branch.branch_id, Branch.branch_name):
        branch_list.append({'id': id, 'name': name})
    return branch_list
...
def load_department():
    ...
    for (id, name) in session.query(Department.department_id, Department.department_name):
        department_list.append({'id': id, 'name': name})
